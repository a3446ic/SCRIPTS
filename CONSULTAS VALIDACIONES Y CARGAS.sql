...
from EXT."EXT_MOVIMIENTO_RECIBOS_HIST" where ESTADOREG = 'NO_CARTERA'  
and TO_NVARCHAR(FECHA_DATOS,'YYYY-MM-DD') <= '2024-10-31' and TO_NVARCHAR(FECHA_DATOS,'YYYY-MM-DD') >= '2024-10-01'
...
CALL "EXT"."SP_GENERAR_RECIBOS_FACTURAS_NOPOST" (20547673299881410, 'N') -- se pasa el plrunseq
...
plrunseq 20547673299881013 Septiembre 2024 PERIODSEQ=2533274790397205
plrunseq 20547673299881410 Octubre 2024 PERIODSEQ=2533274790397206


select * from EXT.CSE_DEBUG where DATETIME> '2024-10-25 12:25'
select PIPELINERUNSEQ,PERIODSEQ  from CS_PLRUN where STARTTIME > '2024-07-23 08:00' and DESCRIPTION like '%CompensateAndPay%'
select top 1 PIPELINERUNSEQ,PERIODSEQ  from CS_PLRUN where DESCRIPTION like '%CompensateAndPay%' and PERIODSEQ = (select top 1 PERIODSEQ from CS_PERIOD where NAME='Septiembre 2024' and REMOVEDATE='2200-01-01' )
----------
CALL "EXT"."SP_GENERAR_RECIBOS_FACTURAS_NOPOST_MEDIADOR" (20547673299878639,'1734-0000','N')
...
----------------------------------
-- POLIZAS que han llegado en MVCAR pero no se han cargado por ser de tipo mov. 2 - renovacion y no existir previamente
----------------------------------
select mf.FECHA_DATOS, mf.NUM_POLIZA,  CAR.ACTIVO, mf.IDPAIS, mf.IDMODALIDAD,   mf.FECHA_EFECTO, mf.FECHA_VENCIMIENTO, mf.IDMEDIADOR, mf.IDSUBCLAVE, mf.IDTIPO_MOV, mf.BATCHNAME, mf.CREATEDATE, mf.ESTADOREG
from EXT.EXT_MOVIMIENTO_CARTERA_CREDITO_HIST mf
LEFT JOIN EXT.CARTERA car on mf.NUM_POLIZA = car.NUM_POLIZA 
where mf.FECHA_DATOS >= '2024-02-01' and mf.IDTIPO_MOV=2 and CAR.ACTIVO is null
...
----------------------------------------------------------------------------------- 
-- EXTRACCION DEL LISTADO DE USUARIOS DEFINIDOS COMO PARTICIPANTS
-----------------------------------------------------------------------------------
select medPrin.POSITIONNAME as CODMEDIADOR, IFNULL(NOMBRE || ' ','')|| "APELLIDO/RAZON_SOCIAL" as MEDIADOR, 
CASE WHEN pay.Payeeid like '%US%' then 'ADICIONAL' else 'PRINCIPAL' end AS TIPO_USUARIO, pay.PAYEEID as Cod_Usuario, 
par.USERID, par.participantemail as EMAIL, IFNULL(par.FIRSTNAME || ' ','')|| par.LASTNAME as USUARIO ,
TO_NVARCHAR(par.EFFECTIVESTARTDATE,'YYYY-MM-DD') as EFFECTIVESTARTDATE, 
TO_NVARCHAR(par.EFFECTIVEENDDATE,'YYYY-MM-DD') as EFFECTIVEENDDATE, BUSINESSUNIT as PAIS 
from TCMP.CS_PARTICIPANT par 
left join TCMP.CS_PAYEE pay ON pay.payeeseq = par.payeeseq AND pay.EFFECTIVESTARTDATE = par.EFFECTIVESTARTDATE AND pay.REMOVEDATE = TO_DATE('22000101', 'yyyymmdd') 
inner join EXT.MODIFICAR_MEDIADOR medPrin on SUBSTRING(pay.PAYEEID, 1, INSTR(pay.PAYEEID, '-') - 1) ||'-0000' = medPrin.PAYEEID 
where par.REMOVEDATE=TO_DATE('22000101','yyyymmdd') and par.ISLAST=1 and par.participantemail is not null /*and pay.Payeeid like '%US%'*/
