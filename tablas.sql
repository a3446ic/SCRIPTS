...
IF (SELECT TABLE_NAME FROM SYS.TABLES where SCHEMA_NAME='EXT' and TABLE_NAME like ('%NOMBRE_TABLA_BKP_' || TO_VARCHAR(CURRENT_DATE, 'YYYYMMDD'))) IS NULL THEN
	EXEC('CREATE COLUMN TABLE EXT.OMBRE_TABLA_BKP_' || TO_VARCHAR(CURRENT_DATE, 'YYYYMMDD') || ' AS (SELECT * FROM EXT.NOMBRE_TABLA)');
...
ALTER TABLE EXT.TABLA_SMM ADD(ID INT);

--BORRAR COLUMNA A UNA TABLA
ALTER TABLE EXT.TABLA_SMM DROP(ID);